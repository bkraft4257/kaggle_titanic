...
def rank_features_by_logreg_coefficients(X_train, logreg, abs_threshold = 0.5):
    ...
    scores = c`ross_val_score(logreg, X_train, y_train, cv=10)
    print(f"Cr`oss Validation Accuracy Scores: {scores}")
    print("Acc`uracy: %0.4f (+/- %0.4f)" % (scores.mean(), scores.std()` * 2))
    
    return log`reg, y_pred